...
bool CALL HGE_Impl::Input_GetKeyState(int key)
{
	...
	if(bActive)
	{
		if(m_kbState[key] ==1) return true;
		else return false;
	}
	else
		return 0;
...
void HGE_Impl::_BuildEvent(int type, int key, int scan, int flags, int x, int y)
{
	...
	Uint8 * kbstate = SDL_GetKeyboardState(NULL);
	POINT pt;
	...
	if (eptr->event.type == INPUT_KEYDOWN || eptr->event.type == INPUT_MBUTTONDOWN) {
		VKey = eptr->event.key;
		Char = eptr->event.chr;		
		m_kbState[eptr->event.key] = 1;
	}
	if (eptr->event.type == INPUT_KEYUP || eptr->event.type == INPUT_MBUTTONUP) 
	{
		m_kbState[eptr->event.key] = 0;
	}

	//else if (eptr->event.type == INPUT_KEYUP)
	//{
	//	//isRepeate = false;
	//	VKey = 0;
	//}
